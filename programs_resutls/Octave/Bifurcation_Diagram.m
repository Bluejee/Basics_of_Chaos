...
for F = 1.35:0.001:1.5
  ...
  n = 1; 
  for i = 1:length(t)
    if abs(t(i) - (2*(n+299)*pi/Omega)) < (dt/2) # To start from 300th period.
      
      Periodic_theta(n) = theta(i);
      n += 1;
  ...
  # Now we have a list of thetas taken periodicaly for periods from 300 to 400
  # We can add this to a column of the Bifurcation_Theta_list
  
  Bifurcation_Theta_list(k) = Periodic_theta';
  k += 1
  toc
endfor
